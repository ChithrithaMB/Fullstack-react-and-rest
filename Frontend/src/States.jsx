...
export const States = ({ children }) => {
    ...
    const [prodquantity,setProdQuantity] = useState("default");
...
const Admin_data21 = { //DOctor table name
  name: name,
  phno: phno,
  email1: email1,
  password1: password1,
};
axios.post('http://localhost:1403/Admin/signup',Admin_data21)
.then((response)=>
{
  console.log(response);
});
...
// database post
const sendtoDB = (e) => {
  const customer_data21 = { //users table name
    username: username,
    mobile: mobile,
    email: email,
    password: password,
  };
  axios.post('http://localhost:1403/Customer/signup',customer_data21)
  .then((response)=>
  {
    console.log(response);
  });
